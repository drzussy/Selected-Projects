...
def test_optimal_tree(capsys):
    ...
    for fn, successes in {
        r'data/special_data.txt': [7 / 30, 10 / 30, 13 / 30, 14 / 30, 14 / 30,
        ...
        for i in range(len(symptoms) + 1):
            if fn.endswith('big_data.txt') and i > 3:
                continue
            tree = ex11.optimal_tree(records, symptoms, i).root
            if CHECK_OPTIMAL_TREE and "special" not in fn:
