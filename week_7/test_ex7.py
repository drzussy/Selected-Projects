...
def test_is_power():
    ...
    for _ in range(20):
        assert not is_power(
            randint(1, 1000), 0), "There isn't edge case for is_power(n, 0)!"
        assert is_power(randint(1, 1000),
                        1), "There isn't edge case for is_power(n, 1)!"
        assert not is_power(1, randint(
            2, 100)), "There isn't edge case for is_power(1, n)!"
    ...
    for i in range(10, 110):
        ...
        assert is_power(
            b, x1), f"is_power(b={b}, x={x1}) is True! {b}^{n1}={x1}"
        assert not is_power(
            b, x2), f"please *check manually* if is_power(b={b}, x={x2}) should be False"
